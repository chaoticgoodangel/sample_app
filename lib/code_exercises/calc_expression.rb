class ExpressionCalculator
  ...
  def initialize(expression)
    ...
    puts "exp: #{@exp} = "
    # puts "paren_paris: #{@paren_pairs}"
  end

  def calculate
    eval_parens if @paren_pairs.length > 0
    res = eval_exp(@exp)
    result = res.to_i == res.to_f ? res.to_i : res.to_f
  ...
  def eval_exp(exp)
    ...
    for idx in 0..exp.length-1
      if exp[idx] == "*" || exp[idx] == "/"
        ...
        if mult_div_vals.length > 0
          if (left_most <= mult_div_vals[-1][:closed])
            # puts "hewwo! uwu #{mult_div_vals[-1]} and left_most = #{left_most}"
            combo = mult_div_vals.pop
            left = combo[:value]
            left_most = combo[:open]
          end
        end
        right = exp.slice(idx + 1..right_most).to_f
        # puts "Idx: #{idx} Left: #{left} Right: #{right}"
        mult_div_vals.push( {
          open: left_most,
          closed: right_most,
    ...
    add_sub_vals = []
    idx = 1
    while idx < exp.length - 1
      puts idx
      if exp[idx] == "+" || exp[idx] == "-"
        if idx < exp.length - 2 && exp[idx + 1] == "-"
          idx += 1
        else
          left_most = (idx - 1).downto(0) { |i| 
            break i if i == 0
          ...
          valid = false
          puts "in right_most add"
          right_most = (idx + 1).upto(exp.length-1) { |i|
            valid ||= !" -".include?(exp[i])
            break i if i == exp.length - 1
            break i - 1 if valid && "+-".include?(exp[i])
          }
          left = exp.slice(left_most...idx).to_f
          if add_sub_vals.length > 0
            if (left_most <= add_sub_vals[-1][:closed])
              puts "hewwo! uwu #{add_sub_vals[-1]} and left_most = #{left_most}"
              combo = add_sub_vals.pop
              left = combo[:value]
              left_most = combo[:open]
            end
          end
          right = exp.slice(idx + 1..right_most).to_f
          puts "Left: #{left} left_most: #{left_most}"
...
#   #   end
#   # end
#   data.to_i == data.to_f ? data.to_i : data.to_f
# end
...
tests.each do |pair|
  correct = calc(pair[0]) == pair[1]
  if !correct
    puts "ohno. that should've been #{pair[1]}"
    break
  end
end